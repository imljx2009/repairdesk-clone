    ...
    <div className="min-h-screen bg-[#f5f5f7] flex items-center justify-center px-4">
      <div className="w-full max-w-[400px] bg-white border border-[#e0e0e8] rounded-2xl p-8 shadow-md">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-[#6b6b80] mt-1">Sign in to RepairDesk</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required className="mt-1 w-full px-3.5 py-3 bg-[#f9f9fb] border border-[#e0e0e8] rounded-xl text-sm focus:border-[#f69323] focus:ring-3 focus:ring-[rgba(246,147,35,0.12)] outline-none transition-all" />
          </div>
        ...
        <p className="text-center text-xs text-[#6b6b80] mt-5">
          ...
          <Link href="/register" className="text-[#f69323] font-semibold hover:underline">Create one</Link>
        </p>
      </div>
    </div>
  );
